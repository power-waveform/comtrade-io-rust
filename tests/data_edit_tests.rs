...
//!
//! 覆盖 `Comtrade` 新增的受控写方法：列替换、通道增删、裁剪行、CFF 写出。
//! 核心断言：编辑后 CFG 通道计数与 DAT 列数保持一致，写出→重解析不列错位。

use comtrade_io::{AnalogChannel, ChannelKind, Comtrade, DataType, Segment, StatusChannel};
...
fn remove_status_channel_syncs_counts() {
    ...
    assert_eq!(ct.config.channels.total, status_before - 1 + ct.config.channels.analog);
...
fn insert_status_channel_syncs_counts() {
    ...
    ct.insert_status_channel(1, ch.clone(), vec![0u8; n]).unwrap();
    assert_eq!(ct.config.channels.status, status_before + 1);
    assert_eq!(ct.config.channels.total, ct.config.channels.analog + status_before + 1);
    assert_eq!(ct.data.as_ref().unwrap().statuses.len(), status_before + 1);
    for (i, ch) in ct.config.statuses.iter().enumerate() {
        assert_eq!(ch.index, i + 1);
    }
    // 非法状态值被拒绝
    let err = ct
        .insert_status_channel(0, ch, vec![7u8; n])
        .unwrap_err();
...
fn to_cff_bytes_and_write_cff_reopenable() {
    ...
    let direct = comtrade_io::CffFile::encode(
        &ct.config,
